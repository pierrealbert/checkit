...
class Ext_Form_Decorator_MrButtons extends Ext_Form_Decorator_MchBox
{
    ...
    public function render($content)
    {
        ...
        foreach ($items as $indx => $itemValue) {
            $elHtml .= $this->getWrapPre($count).'<input type="radio" class="' . $this->getInputClass() . '" value="'.$indx.'" name="'.$element->getFullyQualifiedName().'" id ="'.$element->getFullyQualifiedName().'_'.$indx.'" '.($this->getValue($element) == $indx ? 'checked="checked"' : '').' /> '.
