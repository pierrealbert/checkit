...
class Ext_Debug_Plugin_Doctrine extends ZFDebug_Controller_Plugin_Debug_Plugin implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    /**
    ...
    public function __construct(array $options = array())
    {
        ...
        foreach (Doctrine_Manager::getInstance()->getIterator() as $connection) {
        ...
        foreach ($this->_profilers as $name => $profiler) {
                ...
                foreach ($profiler as $event) {
                    if (in_array($event->getCode(), $this->getQueryEventCodes())) {
                        ...
                        $info = preg_replace('/\b(SELECT|FROM|WHERE|ORDER BY|GROUP BY|LIMIT|LEFT JOIN|INNER JOIN)\b/','<strong><br/>$1</strong>', $info);
                        $info = preg_replace('/\b(AND|OR)\b/','<u>$1</u>', $info);

                        $html .= '<li>[' . round($event->getElapsedSecs()*1000, 2) . ' ms] ';
                        $html .= $info;
